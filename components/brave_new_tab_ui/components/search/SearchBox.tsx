...
const SearchEngineIcon = styled.img`
  width: ${icon.m};
  height: ${icon.m};
`
...
// TODO: Use the built in favicon urls - current icons are unset unless you visit the url
const getIconUrl = (engine: SearchEngineInfo) => `chrome://favicon/size/64@1x/${engine?.origin}`
  ...
  return <SearchInput tabIndex={0} type="text" ref={searchInput} value={query} onInput={e => setQuery(e.detail.value)} placeholder="Search the web privately">
    <Flex slot="left-icon">
      <EnginePicker positionStrategy='fixed' value={keyword} onChange={e => {
        ...
        <div slot="value">
          <SelectedIcon src={getIconUrl(searchEngines.find(s => s.keyword === keyword)!)} />
        </div>
        {searchEngines.map(s => <leo-option value={s.keyword} key={s.keyword}>
          <Option>
            <SearchEngineIcon src={getIconUrl(s)} />{s.name}
          </Option>
        </leo-option>)}
      </EnginePicker>
    </Flex>
